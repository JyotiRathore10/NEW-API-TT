  ...
  try {
    ...
    try {
      const managedEmployeesResult = await pool.request()
        ...
        `);
      user.managedEmployees = managedEmployeesResult.recordset;
    } catch (managerErr) {
      // ManagerEmployee table may not exist, continue without it
      console.warn('Warning: ManagerEmployee table not found or query failed');
    ...
    async function getManagedEmployeesWithDetails(managerId) {
      try {
        ...
        console.warn('Warning: Could not fetch managed employees details');
        return [];
      }
    }
    ...
    try {
      ...
      console.warn('Warning: Could not fetch managed employees');
      user.managedEmployees = [];
    }
